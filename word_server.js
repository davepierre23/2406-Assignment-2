		...
		if(request.method == "POST"){
           ...
           for (var i =0; i< wordArray.length; i++){
             ...
             if (wordArray[i].word==dataObj.word){
               //only change if there one person moving it
            ...
            if (!isWordAlready){
              if(dataObj.text ==""|| dataObj.text ==null){/*do nothing*/ }
                ...
                wordArray.push({word:dataObj.text,x:Math.round(Math.random()*150),y:Math.round(Math.random()*150),isMoving:"NO NAME"});
                }
          }


          if(dataObj.text2) userLoginArray.push({word:dataObj.text2,})
           returnObj = {};
